# Qwen-Image-Edit-2.0 API Reference (DashScope)

Self-contained implementation reference for image editing with `qwen-image-2.0` in Addreams.
...
- `qwen-image-2.0`
- `qwen-image-2.0-2026-03-03` (snapshot)

`qwen-image-2.0` currently has the same capabilities as `qwen-image-2.0-2026-03-03`.
    ...
    "messages": [
      {
        ...
        "content": [
          ...
          { "text": "Use image 1 as the base, apply the color style of image 2." }
        ]
      }
    ]
  },
...
- `model`: must be `qwen-image-2.0` (or snapshot ID).
...
function validateQwenImageEdit20Size(size: string): void {
  const { width, height } = parseSize(size);
  const pixels = width * height;
  const minPixels = 512 * 512;
  const maxPixels = 2048 * 2048;
  if (pixels < minPixels || pixels > maxPixels) {
    throw new Error("qwen-image-2.0 edit size total pixels must be between 512*512 and 2048*2048");
  }
}

export async function editWithQwenImage20(
  req: QwenImageEdit20Request,
  apiKey: string,
  region: "sg" | "bj" = "sg",
): Promise<{ imageUrls: string[]; requestId: string }> {
  if (req.images.length < 1 || req.images.length > 3) {
    throw new Error("qwen-image-2.0 editing requires 1 to 3 input images");
  }

  const n = req.n ?? 1;
  if (n < 1 || n > 6) throw new Error("qwen-image-2.0 editing supports n in [1, 6]");

  if (req.size) validateQwenImageEdit20Size(req.size);
