# Qwen-Image-2.0 API Reference (DashScope)

Self-contained implementation reference for `qwen-image-2.0` in Addreams.
...
`qwen-image-2.0` does not support asynchronous API calls in this API.
...
function validateQwenImage20Size(size: string): void {
  const { width, height } = parseSize(size);
  const pixels = width * height;
  const minPixels = 512 * 512;
  const maxPixels = 2048 * 2048;
  if (pixels < minPixels || pixels > maxPixels) {
    throw new Error("qwen-image-2.0 size total pixels must be between 512*512 and 2048*2048");
  }
}

export async function generateQwenImage20(
  req: QwenImage20Request,
  apiKey: string,
  region: "sg" | "bj" = "sg",
): Promise<{ imageUrls: string[]; requestId: string }> {
  const n = req.n ?? 1;
  if (n < 1 || n > 6) throw new Error("qwen-image-2.0 supports n in [1, 6]");

  const size = req.size ?? "1024*1024";
  validateQwenImage20Size(size);
